#!/usr/bin/env bash
# Spec-decode-under-PP, rebased onto the a9a17e7095 nightly.
# =============================================================================
...
#
# The five were rebased by hand and the whole thing regenerated as a diff against a
# pristine nightly, so what is applied here is one patch keyed to this image rather
# than a patch plus five fixups: 7 files, 596 lines, zero failed hunks at fuzz 0.
#
...
echo "=== dspark-pp-nightly: spec-decode-under-PP for a9a17e7095 ==="
...
if grep -q "The drafter is instantiated only on the last pipeline stage" \
    ...
    echo "[dspark-pp-nightly] already present in this image"
else
    if ! patch -p1 -d "$VLLM_ROOT" --dry-run --forward --fuzz=0 \
         < /configs/patches/k3-dspark-pp-nightly.patch > /tmp/dspp-dry.log 2>&1; then
        echo "[dspark-pp-nightly] FATAL: does not apply to this image" >&2
        cat /tmp/dspp-dry.log >&2
        exit 1
    fi
    patch -p1 -d "$VLLM_ROOT" --forward --fuzz=0 < /configs/patches/k3-dspark-pp-nightly.patch
    echo "[dspark-pp-nightly] applied under $VLLM_ROOT"
fi
...
if 'f"{self.speculative_config.method} with pipeline parallel "\n                        "is not supported."' in mr:
    sys.exit("[dspark-pp-nightly] FATAL: the blanket PP+spec refusal is still there")
...
for src, tok, what in need:
    if tok not in src:
        sys.exit("[dspark-pp-nightly] FATAL: %s missing (%s)" % (what, tok))
...
print("[dspark-pp-nightly] verified: refusal gone, five call sites present, modules load")
PY

echo "=== dspark-pp-nightly: done ==="
