#!/usr/bin/env bash
# k3-merged-v5 + Mooncake under DCP with hybrid attention.
#
# Nothing is patched: the Mooncake DCP support rides in as commits, so this only
# verifies. Same body as the v4 script, wrapping the v5 setup instead.
#
# WHAT v5 CHANGES ON THIS PATH. #51843 added a manager-capability guard to
# fine-grained prefix-cache hits, in the same block as our #50493 carry. The
# rebase kept both; for this model the guard is a no-op because
# FullAttentionManager and MambaManager both declare
# supports_fine_grained_hash_lookup. The v5 setup asserts that resolution.
#
...
# concurrencies, and it finished green. Read the worker log for batch_put
# failures and CPU hit rate before believing any number from these arms.

set -euo pipefail

bash "$(dirname "${BASH_SOURCE[0]}")/kimi-k3-merged-v5.sh"
