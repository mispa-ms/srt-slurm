# Copied verbatim from benchmarks/kernels/benchmark_cp_gather.py, which vllm#51739
# added along with the kernel rewrite it is here to measure.
#
# It is shipped from configs/patches rather than run out of the image because the
# file only exists in images built after #51739 -- the whole point is to run the
# SAME script against the kernel from before and after. That is sound because
# #51739 changed no torch binding: ops.h and torch_bindings.cpp are untouched, so
# every call below resolves identically on both images and only the kernel body
# differs.
#
# The defaults are already this model's shape: --entry-size 576 is Kimi-K3 MLA's
# kv_lora_rank 512 + qk_rope_head_dim 64, and --block-size 64 is what the
# FlashInfer/TokenSpeed MLA backends accept. The variant that matters is
# maybe-dequant: with kv-cache-dtype fp8, is_quantized_kv_cache() is true and the
# DCP chunked-prefill context loop calls ops.gather_and_maybe_dequant_cache.
