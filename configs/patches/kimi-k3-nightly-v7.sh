#!/usr/bin/env bash
# v7: our eight commits on today's nightly, ac7509e2b1 (08-14 06:12).
#
# 59 commits on from v6's base, and five of them land on files our patch also
# touches -- config/speculative.py, models/kimi_k3/nvidia/mla.py,
# v1/core/kv_cache_coordinator.py, v1/core/sched/scheduler.py and the prefill
# TokenspeedMLA backend -- so the v6 patch would not have applied. The eight were
# rebased onto ac7509e2b1 instead and all eight survived: none had landed
# upstream, none went empty. 16 files, +480/-61, still no .cu, .cpp or CMake,
# which is why this can ride a stock image.
#
# vllm#51766 IS PRESENT AND STAYS PRESENT, as on v6. It costs this configuration
# 6.6% at c48 and 8.8% at c70, and it is a correctness fix written for this
# model's own geometry -- without it Core and the worker disagree about the live
# Mamba state column after an external hit. The hunt for where that 8.8% goes is
# a separate track (rounds 1-4); this arm is the ladder.
set -euo pipefail

# The nightly this patch was generated against: vllm/vllm-openai:nightly-3ee2df3033...
readonly PINNED_SHA=ac7509e2b1
readonly PATCH=/configs/patches/k3-ours8-v7-nightly.patch
...
print("=== k3-ours8-v7 verified (today's nightly, no Wei fixes) ===")
PY
