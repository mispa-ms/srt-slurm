#!/usr/bin/env bash
# Spec-decode-under-PP, rebased onto the 2026-08-28 nightly (7c5dc571).
# =============================================================================
...
# forward to 7c5dc571, that version needed exactly one more: 08/28 added
...
echo "=== dspark-pp-828: spec-decode-under-PP for 7c5dc571 ==="
...
if grep -q "The drafter is instantiated only on the last pipeline stage" \
    ...
    if ! patch -p1 -d "$VLLM_ROOT" --dry-run --forward --fuzz=0 \
         < /configs/patches/k3-dspark-pp-901.patch > /tmp/dspp-dry.log 2>&1; then
        echo "[dspark-pp-828] FATAL: does not apply to this image" >&2
        cat /tmp/dspp-dry.log >&2
        exit 1
    fi
    patch -p1 -d "$VLLM_ROOT" --forward --fuzz=0 < /configs/patches/k3-dspark-pp-901.patch
