#!/usr/bin/env bash
# The 2026-08-31 nightly (44fe2a392b) carrying upstream PRs only, plus two lines.
# =============================================================================
...
if not m:
    print(f"[upstream-v1] WARNING: version {v!r} carries no +g<sha>; cannot verify base")
else:
    sha = m.group(1)
    pinned = "${PINNED_SHA}"
    if sha.startswith(pinned) or pinned.startswith(sha):
        print(f"[upstream-v1] base image confirmed: {sha}")
    else:
        print(f"[upstream-v1] WARNING: base is {sha}, this chain was derived against {pinned}")
        print("[upstream-v1] re-check the 'skips' list in this header before trusting a result")
PY
...
if failures:
    for f in failures:
        print(f"[upstream-v1] FATAL: {f}", file=sys.stderr)
    sys.exit(1)

print("[upstream-v1] all 7 assertions passed")
PY
...
print("[upstream-v1] edited modules import cleanly")
PY

echo "=== kimi-k3-upstream-v1: done ==="
