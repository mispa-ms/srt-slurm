#!/usr/bin/env bash
# k3-merged-v3 + Mooncake under DCP with hybrid attention.
#
# Identical to kimi-k3-merged-mooncake.sh except that it wraps the v3 setup, so
# the v3 image marker (vllm/v1/worker/mamba_utils.py, absent from v2) is checked
# before this patch is applied. The 198-line patch itself is byte-identical and
# applies to v3 unchanged: kv_connector/v1/mooncake/ has zero drift between the
# two images.
#
# WHAT v3 CHANGES UNDER US, and why an A/B on v3 needs its own SimpleCPU twin
# rather than a comparison against v2 numbers:
#   #50344 (merged 2026-08-09) gates the divergent hybrid prefix lookup on a new
#   connector property, supports_divergent_local_hybrid_hits. Only NIXL declares
#   it; MooncakeStoreConnector inherits the False default. On v2 every connector
#   got that lookup, so the same config takes the conservative common-prefix path
#   here. Watch GPU prefix cache hit rate: 96.0% for c8 nospec on v2.
#   #48414 (merged 2026-08-10) rewrote the CPU offload layout, but in
#   vllm/v1/kv_offload/ and kv_connector/v1/offloading/ -- not in mooncake/. The
#   measured v2->v3 SimpleCPU delta is -2.7%..+2.4% with GPU hit unchanged, i.e.
#   noise, so the twin baseline is comparable.
#
...
bash /configs/patches/kimi-k3-merged-v3.sh
