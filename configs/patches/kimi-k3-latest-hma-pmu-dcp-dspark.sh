#!/bin/bash
# kimi-k3-latest-dcp.sh plus DCP under DSpark speculative decoding.
# The second track, on a current nightly. See kimi-k3-latest-dcp.sh for why.
#
...
#
# WHAT IT CARRIES. k3-dcp-dspark-fix-latest.patch, four changes, all ours:
#
...
#
# ON THIS TRACK THERE IS NO BACKEND SPLIT. vllm#50911 gives
# TokenspeedMLAMetadataBuilder supports_non_causal_multi_token_decode=True, so
# the draft stays on TOKENSPEED_MLA with the target and keeps
# reorder_batch_threshold=5 instead of being clamped to 1. The config must NOT
# carry speculative-config.attention_backend; leaving the old FLASHINFER_MLA
# override in place would re-create the capture failure this track exists to
# avoid.
#
...
bash /configs/patches/kimi-k3-latest-dcp.sh

echo "=== k3-dcp-dspark-fix (ours, on top of the DCP stack) ==="
FIX_FILE=/configs/patches/k3-dcp-dspark-fix-latest.patch
