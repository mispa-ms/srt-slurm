#!/bin/bash
# Kimi-K3 AGG v2 + DSpark under DCP, WITH vllm#50532 on top.
#
...
FIX=/configs/patches/k3-dspark-on-aggv2-pr50532.patch
...
# vllm#50532 is carried in this patch, so require its fix to be present: the
# shape-only classifier must be gone from the scheduler path.
assert 'get_uniform_decode_token_count' in read('v1/worker/gpu/model_runner.py'), (
    ...
    'shape alone, so a prompt chunk of 1+num_speculative_tokens tokens replays the '
    'spec-verify decode graph'
)
