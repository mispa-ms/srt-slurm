#!/usr/bin/env bash
# The disagg Mooncake setup, nsys, a py-spy stack dumper, and shm index logging.
# =============================================================================
...
if ! command -v nsys >/dev/null 2>&1; then
    ...
    for pkg in nsight-systems-2025.6.3 nsight-systems-2025.3.1 nsight-systems nsight-systems-cli; do
        if apt-get install -y --no-install-recommends "$pkg"; then
            echo "[k3-mooncake-shmdbg] installed $pkg"
            break
        fi
        echo "[k3-mooncake-shmdbg] package $pkg unavailable, trying next"
    done
fi

command -v nsys >/dev/null 2>&1 || {
    echo "[k3-mooncake-shmdbg] FATAL - nsys still not on PATH after install attempts" >&2
    exit 1
}
nsys --version
# ---------------------------------------------------------------------------
