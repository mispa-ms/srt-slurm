...
class TestRouterlessMultiNodeTopology:
    def test_two_node_agg_yields_one_process_per_node(self):
        ...
        # Assert the property, not the number. This branch's port allocator gives
        # the leader an allocated HTTP port rather than the fixed public one, and
        # benchmark_stage selects by is_leader and skips ports <= 0 -- which is
        # what "rank 0 serves, followers are headless" has to mean here.
